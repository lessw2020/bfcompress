...
std::tuple<torch::Tensor, torch::Tensor> compress_bfloat16(torch::Tensor input) {
    ...
    if (error != cudaSuccess) {
        std::cerr << "CUDA error in compression: " << cudaGetErrorString(error) << std::endl;
        throw std::runtime_error("CUDA error in compression");
    }
...
torch::Tensor decompress_bfloat16(torch::Tensor input, torch::Tensor bit_size, int64_t output_size) {
    ...
    cudaError_t error = cudaGetLastError();
    if (error != cudaSuccess) {
        std::cerr << "CUDA error in decompression: " << cudaGetErrorString(error) << std::endl;
        throw std::runtime_error("CUDA error in decompression");
    }
